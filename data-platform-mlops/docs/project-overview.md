...
```
data-platform-mlops        <- Root directory. Both monorepo and polyrepo are supported.
data-platform-mlops        <- Root directory. Both monorepo and polyrepo are supported.
│
├── data_platform_mlops       <- Contains python code, notebooks and ML resources related to one ML project. 
│   │
